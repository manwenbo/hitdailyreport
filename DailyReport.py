...
#需要修改
stu_number_0 = dateRecorder.readline()[:-1]
stu_number = [stu_number_0]  # 学号
stu_password_0 = dateRecorder.readline()[:-1]
stu_password = [stu_password_0] # 密码
stu_name = 'dailyreport'  # 你的姓名的拼音，仅用来命名截图，可以不改，这里不要用中文
sender = dateRecorder.readline()[:-1] # 你的邮箱
receiver = dateRecorder.readline()[:-1] # 你的邮箱
password_email = dateRecorder.readline()[:-1] #授权码
# 输入下载的edgedriver放置的路径，建议使用双斜杠
driver_url = dateRecorder.readline()[:-1]
Reason=dateRecorder.readline()[:-1] #出校申请的理由
...
if needGoOutRequst=='需要出校申请':
    ...
    time.sleep(0.1)
    driver.find_element_by_xpath('//*[@id="checkbox7"]').click()
    time.sleep(0.1)
